...
# Data
data_train, labels_train = torch.load(f'./xor_datasets/{TASK}_{SEQ_LEN}_train.pt')
...
data_val, labels_val = torch.load(f'./xor_datasets/{TASK}_{SEQ_LEN}_val.pt')
val_set = DatasetCreator(data_val, labels_val)
val_loader = DataLoader(val_set, batch_size=BATCH, shuffle=False, drop_last=False)

data_test, labels_test = torch.load(f'./xor_datasets/{TASK}_{SEQ_LEN}_test.pt')
